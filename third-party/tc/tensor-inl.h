...
inline std::unique_ptr<DLTensorType, DLTensorDeleter>
makeDLTensor(DLContext ctx, DLDataType dtype, const std::vector<T> &sizes,
             const std::vector<T> &strides = std::vector<T>(),
             decltype(DLTensorType().data) data = nullptr,
             uint64_t byteOffset = 0) {
    static_assert(std::is_convertible<T, int64_t>::value,
...
template <typename DLTensorType>
inline std::unique_ptr<DLTensorType, DLTensorDeleter>
makeDLTensorHelper(const DLTensor *ptr) {
    ...
    return makeDLTensor<DLTensorType>(ptr->ctx, ptr->dtype, sizes, strides,
...
inline DLTensorUPtr makeDLTensor(const TensorInfo &tensor) {
    return detail::makeDLTensor<DLTensor>(DLContext{kDLCPU, 0}, tensor.dtype,
                                          tensor.shape, tensor.strides, nullptr,
                                          tensor.alignment);
}
...
inline DLTensorUPtr
makeDLTensor(DLContext ctx, DLDataType dtype, const std::vector<T> &sizes,
             const std::vector<T> &strides, void *data, uint64_t byteOffset) {
    return detail::makeDLTensor<DLTensor>(ctx, dtype, sizes, strides, data,
                                          byteOffset);
}
...
inline DLConstTensorUPtr makeDLConstTensor(const TensorInfo &tensor) {
    return detail::makeDLTensor<DLConstTensor>(
        DLContext{kDLCPU, 0}, tensor.dtype, tensor.shape, tensor.strides,
        nullptr, tensor.alignment);
}

template <typename T>
inline DLConstTensorUPtr
makeDLConstTensor(DLContext ctx, DLDataType dtype, const std::vector<T> &sizes,
                  const std::vector<T> &strides, const void *data,
                  uint64_t byteOffset) {
    return detail::makeDLTensor<DLConstTensor>(ctx, dtype, sizes, strides, data,
                                               byteOffset);
}
...
template <typename T>
std::vector<DLConstTensorUPtr>
makeDLConstTensorVector(const std::vector<T> &ptrs) {
...
makeTensorInfoVector(const std::vector<DLTensorPtrType> &ts) {
    std::vector<TensorInfo> res;
    res.reserve(ts.size());
    std::transform(ts.begin(), ts.end(), std::back_inserter(res),
                   [](DLTensorPtrType t) { return TensorInfo(t); });
    return res;
}

inline std::vector<const DLTensor *>
extractRawPtrs(const std::vector<DLTensorUPtr> &uptrs) {
...
inline std::vector<const DLConstTensor *>
extractRawPtrs(const std::vector<DLConstTensorUPtr> &uptrs) {
...
inline std::string toString(const DLDataType &t) {
    if (t.lanes != 1) {
        throw std::invalid_argument{"NYI: toString for >1 lanes"};
    }
    switch (t.code) {
    ...
    throw std::invalid_argument{
        "NYI: toString for type: " + std::to_string(t.code) +
...
inline std::string toString(const DLTensor &t) {
    ...
    ss << "DLTensor@" << t.data << ":\n";
    std::string res;
    google::protobuf::TextFormat::PrintToString(TensorInfo(&t).toProtobuf(),
                                                &res);
    ss << res;
    return ss.str();
}
